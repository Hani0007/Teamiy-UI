...
use App\Http\Controllers\Web\DeleteAdminAccount;
...
Route::get('/test-role', function () {
    ...
    $role = ModelsRole::firstOrCreate(['name' => 'super-admin', 'guard_name' => 'admin']);

    // create permission if not exists
    $permission = Permission::firstOrCreate(['name' => 'view_dashboard', 'guard_name' => 'admin']);
...
Route::group([
    ...
    Route::get('/admin/invoice/{id}', function ($subscriptionId) {
        ...
        $data = [
            ...
            'cycle'      => ucfirst($subscription->cycle ?? 'N/A'),
            'employees'  => $subscription->total_employees ?? 0,
            'amount'     => number_format($subscription->amount, 2),
            'currency'   => $subscription->admin?->company?->currency?->symbol ?? '$',
        ];

        $pdf = Pdf::loadView('emails.admin_invoice', $data);
        $fileName = 'invoice-' . $subscriptionId . '.pdf';

        return $pdf->download($fileName);

        // return view('emails.admin_invoice')->with([
        //     'adminName'  => $subscription->admin?->name,
        //     'planName'   => $subscription->plan?->name,
        //     'cycle'      => ucfirst($subscription->cycle ?? 'N/A'),
        //     'employees'  => $subscription->total_employees ?? 0,
    ...
    Route::get('/admin/delete_account_form', [DeleteAdminAccount::class, 'index'])->name('admin.delete_account');
    Route::post('/admin/send_delete_request', [DeleteAdminAccount::class, 'sendRequest'])->name('admin.delete_request');
    // Route::get('/account/delete', [DeleteAdminAccount::class, 'deleteAccount'])->name('delete.admin.safe');

    Route::get('/delete_admin', function (Request $request) {
        if (! $request->hasValidSignature()) {
            return redirect()->route('admin.delete_account')->with('error', 'link_expired_or_invalid');
        }

        $email = $request->query('email');

        if (!$email) {
            return redirect()->route('admin.delete_account')->with('error', 'Missing email in request');
        }

        $user = Admin::where('email', $email)->first();

        if (!$user) {
            return redirect()->route('admin.delete_account')->with('error', 'unauthorized_access');
        }
        ...
        return view('admin.deleted-success');
    ...
    Route::group([
        ...
        Route::get('register', [AdminAuthController::class, 'register'])->name('company-register');
        Route::post('register', [AdminAuthController::class, 'registerCompany'])->name('company.register.process');
        Route::post('logout', [AdminAuthController::class, 'logout'])->name('logout');
        Route::get('verify', [AdminAuthController::class, 'showVerificationForm'])->name('verify.show');
        Route::post('verify', [AdminAuthController::class, 'verifyCode'])->name('verify.perform');
        Route::get('verification-success', [AdminAuthController::class, 'showVerificationSuccess'])->name('verification.success');
        Route::post('resend-otp', [AdminAuthController::class, 'resendOtp'])->name('verify.resend');

        Route::middleware([
            'auth.multiple:admin,web'
        ])->group(function () {
            Route::post('/stripe/create_subscription', [StripeController::class, 'createSubscription'])->name('stripe.create.subscription');
            Route::post('/stripe/payment-success', [StripeController::class, 'paymentSuccess'])->name('stripe.success');
            Route::get('/subscription/thank-you', [StripeController::class, 'thankYou'])->name('subscription.thankyou');
        });
        ...
        Route::middleware([
            ...
            Route::middleware(['role:super-admin'])
                ->group(function () {
                    Route::get('/subscription_plans', function () {
                        $plans = Package::with(['packageModules.module'])->where('id', '!=', 1)
                            ->get();
                        $token = auth()->user()->createToken('web')->accessToken;
                        $user = auth()->user();
                        $planName = $user->plan->name;

                        return view('admin.subscription.plans', compact('plans', 'token', 'planName'));
            ...
            Route::get('/admin/profile_edit/{id}', [AdminController::class, 'profileEdit'])->name('profile_edit');
            Route::post('/admin/profile/{id}/update', [AdminController::class, 'updateProfile'])->name('profile_update');
            ...
            Route::get('/fetch_departments', [UserController::class, 'fetchDepartment'])->name('fetch.departments');
            Route::get('/fetch_designations', [UserController::class, 'fetchDesignations'])->name('fetch.designations');
            Route::get('/fetch_office_timings', [UserController::class, 'fetchOfficeTImings'])->name('fetch.office.timings');
            ...
            Route::get('/department/users', [AssetController::class, 'users'])->name('department.users');
            Route::post('/assign_asset', [AssetController::class, 'assignAsset'])->name('assign.asset');
            Route::post('/return_asset', [AssetController::class, 'returnAsset'])->name('return.asset');
            ...
            Route::get('/payroll/salary-slip/{id}', [EmployeeSalaryController::class, 'downloadSalarySlip'])->name('payroll.salary.slip');
            ...
            Route::resource(
                'termination-types',
                TerminationTypeController::class,
                ['except' => ['destroy', 'create', 'edit']]
            );
