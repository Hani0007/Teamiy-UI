        ...
        try {
            ...
            DB::commit();
                ...
                if($project && $validatedData['notification'] == 1){
                    ...
                    $addedUserIds = array_unique($validatedData['assigned_member']);
    ...
    public function show($id)
    {
        ...
        try {
            ...
            return view($this->view . 'show',
                compact('projectDetail','projectMemberIds','employees',
                    'assignedMember',
                    'images',
                    'files')
            );
        ...
        try {
            ...
            if($projectDetail && $validatedData['notification'] == 1){
                ...
                if(strtotime($today) <= strtotime($end)){
                    ...
                    // project assign
                    $addedUserIds = array_unique(array_merge($addedEmployeeIds,[]));
                    $addMessage = __('message.project_assign',['name'=>$validatedData['name'],'deadline'=>$validatedData['deadline']]);
                    $this->sendNotification($addedUserIds, $addMessage, $projectId);


                    // remove from project
                    $removedUserIds = array_unique(array_merge($removedEmployeeIds,[]));
                    ...
                    // project change
                    $otherUserIds = array_unique(array_merge($remainingEmployeeIds,[]));
