...
class AssetController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            $select = ['*'];
            $with = ['type:id,name','latestAssignment.user:id,name', 'branch', 'branch.departments'];
    ...
    {
        return User::where('department_id', $request->department_id)
                    ->select('id', 'name')
                    ->get();
    }
    ...
    public function assignAsset(Request $request)
    {
       ...
       try{
            ...
            DB::rollBack();
            return redirect()->back()->with(['error' => $ex->getMessage()], 400);
       }
    }
    ...
    public function returnAsset(Request $request)
    {
        try {
            ...
            if (!$assetAssigned) {
                return redirect()->back()->with(['error' => __('record_not_found')], 404);
            }

            $assetAssigned->update($validatedData);
            $isWorking = $validatedData['return_condition'] === 'working' ? 'yes' : 'no';
            $assetAssigned->asset->update([
