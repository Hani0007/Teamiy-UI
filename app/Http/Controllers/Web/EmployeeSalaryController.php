...
//    }
    ...
    public function payroll(Request $request): Factory|View|RedirectResponse|Application
    {
        try{
            ...
            // }else {
            $payrolls = $this->generatePayrollService->getCurrentEmployeeSalaries($filterData);
            // }
            ...
            $salary_from =$dateInAd['start_date'];
            $salary_to =$dateInAd['end_date'];

            return view($this->view.'payroll', compact('payslipStatus', 'salary_from', 'salary_to', 'salaryCycles','payrolls', 'filterData','paymentMethods','currency','employees','branches','months','isBSDate','currentNepaliYearMonth'));
        }catch(Exception $exception){
            return redirect()
                ->back()
                ->with('danger', $exception->getMessage());
    ...
    public function downloadSalarySlip($id)
    {
        ...
        $pdf = Pdf::loadView('admin.payroll.employeeSalary.salary-slip', compact(
            'payroll',
            'salary_from',
            'salary_to',
            'currency'
    ...
    public function makePayment($payslipId, Request $request)
    {
        try{
            ...
            $payroll = GeneratedPayroll::where('id', $payslipId)->first();
