...
class LeaveApiController extends Controller
{
    ...
    public function getAllLeaveRequestOfEmployee(Request $request): JsonResponse
    {
        try{
            $filterParameter = [
                'leave_type' => $request->leave_type_id ?? null,
            ...
            // if( isset($request) && ($request->leave_type_id == '' || $request->leave_type_id == 0)){
            if($request->type === 'short_leave'){
                ...
                $timeLeaveRequests = $this->timeLeaveService->getAllTimeLeaveRequestOfEmployee($filterParameter);
                $mergedCollection = $timeLeaveRequests;
            }else{
                $getAllLeaveRequests =  $this->leaveService->getAllLeaveRequestOfEmployee($filterParameter);
        ...
        try {
            ...
            if(isset($attendance))
            {
                return response()->json([
                    'message' => 'Attendance already marked by employee on your leave requested date.',
                ], 400);
            }
...
//            $this->authorize('leave_request_create');
            ...
            $validatedData['referred_by'] = AppHelper::getAuthUserId();
