...
class AttendanceHelper
{
    ...
    public static function isHolidayOrWeekendOnCurrentDate(): bool
    {
        ...
        $holiday = Holiday::whereDate('event_date', $date)->where('company_id', AppHelper::getWebAdminCompanyId())
    ...
    public static function isHolidayOrWeekend($startDate, $endDate): bool
    {
        ...
        if ($start->eq($end)) {
            ...
            $holiday = Holiday::whereDate('event_date', $currentDate)->where('company_id', $companyId)->count();
            ...
            $weekend = in_array($weekDay, $company->weekend);
...
//            ->get();
     ...
     * @throws Exception
     */
    public static function calculateNightWorkedHour($checkOutDateTime, $checkInDateTime, $employeeId): array
    {
    ...
    {
        $shift = User::with(['officeTime:id,opening_time,closing_time'])
            ->select('id', 'office_time_id')
            ->find($employeeId);

        if (!$shift || !$shift->officeTime) {
            return [
                'workedHours' => 0,
                'overtime' => 0,
                'undertime' => 0,
        ...
        $workingTime = self::calculateTime($shift->officeTime->closing_time, $shift->officeTime->opening_time);
        ...
        if ($workedTime > $workingTime) {
            $overtime = $workedTime - $workingTime;
        } elseif ($workedTime < $workingTime) {
            $undertime = $workingTime - $workedTime;
        }
     ...
     * @throws Exception
     */
    public static function calculateTime($outTime, $inTime): float
    {
