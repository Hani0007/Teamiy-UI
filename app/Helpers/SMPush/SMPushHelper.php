...
class SMPushHelper
{
    ...
    ): void
    {
        ...
        try {
            $tokens = $type == 'group_chat'
                ? self::getEmployeeFCMTokensForSending($usernames)
                : self::getEmployeeByUsernameFCMTokensForSending($usernames);

            Log::info('FCM Tokens Found:', [
                'tokens' => $tokens
            ]);
            ...
            SMPushNotification::smSend(
                ...
                data: [
                    'title'             => $title,
                    'conversation_id'   => $conversation_id,
                    'sender_name'       => AppHelper::getAuthUser()->name,
                    'sender_image'      => AppHelper::getAuthUser()->avatar
                        ? asset(User::AVATAR_UPLOAD_PATH . AppHelper::getAuthUser()->avatar)
                        : asset('assets/images/img.png'),
                    'message'           => $message,
                    'type'              => $type,
                    'project_id'        => $project_id,
                    'sender_username'   => AppHelper::getAuthUser()->username,
                ],
                recipients: $tokens
            );
