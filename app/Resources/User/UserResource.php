...
class UserResource extends JsonResource
{
    public function toArray($request)
    {
        return [
            ...
            'office_time' => (!empty($this?->officeTime?->opening_time) && !empty($this?->officeTime?->closing_time))
                                ? ($this?->officeTime?->opening_time . ' - ' . $this?->officeTime?->closing_time)
                                : null,
            'branch' => ucfirst($this?->branch?->name),
            'department' => ucfirst($this?->department?->dept_name),
            'designation' => ucfirst($this?->post?->post_name),
            'role' => ucfirst($this->role_name),
