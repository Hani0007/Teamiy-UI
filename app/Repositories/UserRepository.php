...
class UserRepository
{
    ...
    public function getAllVerifiedEmployeeOfCompany($select = ['*'], $with = [])
    {
       ...
       return User::select($select)->with($with)
            ->where('status', 'verified')
            ->where('is_active', self::IS_ACTIVE)
            ->where('company_id', AppHelper::getWebAdminCompanyId())
           ->get();
    }
    ...
    public function store($validatedData)
    {
        ...
        if(isset($validatedData['role_id']) && !is_numeric($validatedData['role_id'])){
            $roleId = DB::table('roles')
                ->where('name', $validatedData['role_id'])
                ->value('id');
            if($roleId){
        ...
        if (isset($validatedData['employee_document'])) {
            foreach ($validatedData['employee_document'] as $document) {
                $documents[] = $this->storeEmpDocuments($document, 'uploads/user/emp-documents');
            }
        }

        $empDocuments = [
            'employee_id'       => $user->id,
            'employee_contract' => $empContract,
            'employee_document' => empty($documents) ? null : $documents,
        ];

        EmployeeDocument::updateOrCreate(['employee_id' => $user->id], $empDocuments);
    ...
    public function update($userDetail, $validatedData)
    {
        ...
        $employeeDoc = EmployeeDocument::where('employee_id', $userDetail->id)->first();

        $empContract = $employeeDoc?->employee_contract;
        $documents   = $employeeDoc?->employee_document ? $employeeDoc->employee_document : [];

        // If new contract uploaded → remove old one, then upload new
        if (isset($validatedData['upload_contract'])) {
            if ($employeeDoc && $employeeDoc->employee_contract) {
                $this->removeImage('uploads/user/emp-documents', $employeeDoc->employee_contract);
            }
        ...
        // If new documents uploaded → remove old ones, then upload new
        if (isset($validatedData['employee_document'])) {
            if ($employeeDoc && $employeeDoc->employee_document) {
                $oldDocs = is_array($employeeDoc->employee_document)
                    ? $employeeDoc->employee_document
                    : json_decode($employeeDoc->employee_document ?? '[]', true);
                foreach ($oldDocs as $oldDoc) {
                    $this->removeImage('uploads/user/emp-documents', $oldDoc);
                }
            }

            $documents = []; // reset and replace with new ones
            foreach ($validatedData['employee_document'] as $document) {
                $documents[] = $this->storeEmpDocuments($document, 'uploads/user/emp-documents');
            }
        }
        ...
        EmployeeDocument::updateOrCreate(['employee_id' => $userDetail->id], $empDocuments);
...
//    }
    ...
    public function getActiveEmployeesByDepartments($departmentIds, $select = ['*'])
    {
        return User::select($select)
            ->where('status', 'verified')
            ->where('is_active', self::IS_ACTIVE)
