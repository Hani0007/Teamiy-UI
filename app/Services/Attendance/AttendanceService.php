...
//            }
    ...
    public function findEmployeeTodayAttendanceDetail($userId, $attendanceDate = null, $select = ['*'])
    {
        return $this->attendanceRepo->findEmployeeTodayCheckInDetail($userId, $attendanceDate, $select);
    }
    ...
    public function newCheckIn($validatedData)
    {
        ...
        $checkInAt = $validatedData['check_in_at'] ?? Carbon::now()->toTimeString();
        ...
        if ($shift && isset($shift->opening_time)) {
            ...
            if ($timeLeave && $checkInAt->between(
                Carbon::parse($timeLeave->start_time),
                Carbon::parse($timeLeave->end_time)
            )) {
                // Move check-in to after leave ends
                $checkInAt = Carbon::parse($timeLeave->end_time);
            }

        }

        $validatedData['attendance_date'] = $validatedData['attendance_date'] ?? Carbon::now()->format('Y-m-d');
    ...
    {
        $checkOut = isset($validatedData['check_out_at']) ? $validatedData['check_out_at'] : Carbon::now()->toTimeString();
        ...
        if ($shift && isset($shift->closing_time)) {
            ...
            $checkOutAt = isset($validatedData['check_out_at']) ? $validatedData['check_out_at'] : Carbon::createFromTimeString(now()->toTimeString());
