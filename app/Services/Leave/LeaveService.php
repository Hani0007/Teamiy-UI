...
class LeaveService
{
    ...
    public function storeLeaveRequest($validatedData)
    {
            ...
            if (isset($validatedData['avatar'])) {
                 $validatedData['avatar'] = $this->storeImage(
                     ...
                     LeaveRequestMaster::ATTACHMENT_UPLOAD_PATH,
    ...
    private function checkEmployeeLeaveRequest($validatedData): void
    {
            ...
            if ($totalLeaveAllocated > 0 &&
                ((int)$validatedData['no_of_days'] + (int)$totalLeaveTakenTillNow > $totalLeaveAllocated)
            ) {
                throw new Exception(__('message.leave_exceed_error', [
                    'day' => ((int)$validatedData['no_of_days'] + (int)$totalLeaveTakenTillNow - $totalLeaveAllocated),
                    'name' => $leaveType->name
                ]), 400);
            }
