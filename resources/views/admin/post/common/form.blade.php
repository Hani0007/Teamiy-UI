<div class="row align-items-center">
    ...
    <div class="col-lg-4 col-md-6 mb-4">
        ...
        <select class="form-select" id="department_id" name="dept_id" required>
            <option {{isset($postDetail) ? '' : 'selected'}} disabled>{{ __('index.select_department') }}</option>

            @if(isset($companyDetail))
                @foreach($companyDetail->departments as $department)
                    <option value="{{ $department->id }}"
                        {{ (isset($postDetail) && $postDetail->dept_id == $department->id) ? 'selected' : '' }}>
