            ...
            try {
                ...
                if (!response || !response.data || response.data.length == 0) {
                    $('#department_id').append(
                        '<option disabled>{{ __('index.no_departments_found') }}</option>');
                    return;
                }
                ...
                $('#department_id').append(
                    '<option disabled>{{ __('index.error_loading_departments') }}</option>');
            }
        };

        const preloadEmployees = async () => {
            const selectedDepartments = $('#department_id').val() || [];
            const previouslySelectedEmployees = $('#team_meeting').val() ||
        []; // Keep track of currently selected employees

            if (selectedDepartments.length === 0) {
                if ($('#team_meeting option').length === 0) {
                    $('#team_meeting').empty().append(
                        '<option disabled>{{ __('index.no_employees_found') }}</option>');
                }
                return;
            }
            ...
            try {
                ...
                const currentOptions = Array.from($('#team_meeting option')).map(option => option
                    .value);

                // Append new employees from fetched data
                data.forEach(employee => {
                    const isSelected = employeeIds.includes(employee.id) ||
                        previouslySelectedEmployees.includes(employee.id.toString());
