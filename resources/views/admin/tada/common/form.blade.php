...
<div class="row">
    ...
    <div class="col-lg-4 col-md-6 mb-4">
        ...
        <select class="form-select" id="branch_id" name="branch_id">
            ...
            @if(isset($companyDetail))
                @foreach($companyDetail->branches()->get() as $key => $branch)
                    <option value="{{$branch->id}}" {{ (isset($tadaDetail) && ($tadaDetail->branch_id ) == $branch->id) ? 'selected': '' }}>
                        {{ucfirst($branch->name)}}
                    </option>
                @endforeach
            @endif
    ...
    <div class="col-lg-4 col-md-6 mb-4">
        ...
        <select class="form-select" id="department_id" name="department_id">
            @if(isset($tadaDetail))
                @foreach($filteredDepartment as $department)
                    <option value="{{ $department->id }}" {{ $department->id == $tadaDetail->department_id ? 'selected' : '' }}>
                        {{ ucfirst($department->dept_name) }}
                    </option>
                @endforeach
            @else
    ...
    <div class="col-lg-4 col-md-6 mb-4">
        <label for="title" class="form-label"> {{ __('index.title') }} <span style="color: red">*</span></label>
        <input type="text" class="form-control" id="title" name="title" required value="{{ ( isset($tadaDetail) ? $tadaDetail->title: old('title') )}}" autocomplete="off" placeholder="Enter TADA Title">
    </div>

    <div class="col-lg-4 col-md-6 mb-4">
        <label for="expense" class="form-label"> {{ __('index.total_expense') }} <span style="color: red">*</span> </label>
        <input type="number" min="0" class="form-control" id="total_expense" name="total_expense" required value="{{ ( isset( $tadaDetail) ? $tadaDetail->total_expense: old('total_expense') )}}" autocomplete="off" >
    </div>
    ...
    <div class="row px-0 mx-0">
        ...
        <div class="col-lg-6 mb-4">
            ...
            <div class="card p-3 mb-3" style="background: #fdfdfd; border: 1px solid #eee;">
                <div class="row">
                    @foreach($attachments as $key => $data)
                        <div class="col-3 mb-2">
                            <div class="position-relative border rounded p-1">
                                @if(!in_array(pathinfo($data->attachment, PATHINFO_EXTENSION), ['docx','pdf','doc','xls','txt']))
                                    <img class="w-100 rounded" src="{{ asset(\App\Models\TadaAttachment::ATTACHMENT_UPLOAD_PATH.$data->attachment) }}" style="height: 50px; object-fit: cover;">
                                @else
                                    <div class="text-center py-2"><i data-feather="file-text"></i></div>
                                @endif
                                <a class="delete position-absolute top-0 end-0 bg-danger text-white rounded-circle" 
