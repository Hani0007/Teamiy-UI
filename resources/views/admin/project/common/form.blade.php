...
<div class="row">
    ...
    <div class="col-lg-6 col-md-6 mb-4">
        ...
        <select class="form-select" id="branch_id" name="branch_id">
            ...
            @foreach($branch as $value)
                <option value="{{ $value->id }}" {{ (isset($projectDetail) && $projectDetail->branch_id == $value->id) ? 'selected' : '' }}>
                    {{ ucfirst($value->name) }}
                </option>
            @endforeach
        </select>
    ...
    @if(\App\Helpers\AppHelper::ifDateInBsEnabled())
        ...
        <div class="col-lg-6 col-md-6 mb-4">
            <label for="deadline" class="form-label">{{ __('index.project_deadline') }} </label>
    ...
    @else
        ...
        <div class="col-lg-6 col-md-6 mb-4">
            <label for="deadline" class="form-label">{{ __('index.project_deadline') }} </label>
            <input type="date" class="form-control" id="deadline" name="deadline" value="{{ ( isset( $projectDetail) ?  $projectDetail->deadline: old('deadline') )}}"
                   autocomplete="off" >
        </div>
    @endif
    ...
    {{-- <div class="col-lg-6 col-md-6 mb-4">
        <label for="cost" class="form-label">{{ __('index.project_cost') }} <span style="color: red">*</span></label>
        <input type="number" class="form-control" id="cost" name="cost" required value="{{ ( isset( $projectDetail) ?  $projectDetail->cost: old('cost') )}}"
               autocomplete="off" >
    </div> --}}
    ...
    <div class="col-lg-6 col-md-6 mb-4">
        ...
        <input type="number" step="0.5" class="form-control" id="estimated_hours" name="estimated_hours" placeholder="{{__('enter_estimated_hours')}}"  value="{{ ( isset( $projectDetail) ?  $projectDetail->estimated_hours: old('estimated_hours') )}}"
               autocomplete="off" >
    </div>

    <div class="col-lg-6 col-md-6 mb-4">
        <div class="team_leader mb-4">
            <label for="department_ids" class="form-label">{{ __('index.department') }} <span style="color: red">*</span></label>
            <select class="col-md-12 from-select" id="department_ids" name="department_ids[]" multiple="multiple" required>
            </select>
        </div>
