<div class="row">
    ...
    <div class="col-lg-4 col-md-6 mb-4">
        ...
        <input type="text" class="form-control" id="company_name" value="{{ isset($company) ? $company->name : '' }}"
            disabled>
    ...
    <div class="col-lg-4 col-md-6 mb-4">
        ...
        <input type="text" class="form-control" id="name" required name="name"
            value="{{ isset($branch) ? $branch->name : '' }}" autocomplete="off" placeholder="">
    </div>
    ...
    <div class="col-lg-4 col-md-6 mb-4">
        ...
        <select class="form-select" id="exampleFormControlSelect1" name="branch_head_id">
            <option value="" {{ !isset($branch) ? 'selected' : '' }} disabled>
                {{ __('index.select_branch_head') }}</option>
            @foreach ($users as $key => $user)
                <option value="{{ $user->id }}"
                    {{ isset($branch) && $branch->branch_head_id == $user->id ? 'selected' : '' }}>
    ...
    <div class="col-lg-4 col-md-6 mb-4">
        ...
        <input type="text" class="form-control" id="address" required name="address"
            value="{{ isset($branch) ? $branch->address : old('address') }}" autocomplete="off">
    ...
    <div class="col-lg-4 col-md-6 mb-4">
        <label for="phone" class="form-label">{{ __('index.phone_number') }} <span
    ...
    <div class="col-lg-4 col-md-6 mb-4" style="display: none;">
        ...
        <input type="text" class="form-control" id="branch_location_latitude" name="branch_location_latitude"
            value="{{ isset($branch) ? $branch->branch_location_latitude : old('branch_location_latitude') }}"
            autocomplete="off" placeholder="{{ __('index.enter_branch_location_latitude') }}">
    </div>

    <div class="col-lg-4 col-md-6 mb-4" style="display: none;">
        <label for="branch_location_longitude" class="form-label">{{ __('index.branch_location_longitude') }}</label>
        <input type="text" class="form-control" id="branch_location_longitude" name="branch_location_longitude"
            value="{{ isset($branch) ? $branch->branch_location_longitude : old('branch_location_longitude') }}"
            autocomplete="off" placeholder="{{ __('index.enter_branch_location_longitude') }}">
    </div>
    ...
    <div class="col-lg-4 mb-4">
        ...
        <select class="form-select" id="exampleFormControlSelect1" name="is_active">
            <option value="" {{ !isset($branch) ? 'selected' : '' }} disabled>{{ __('index.select_status') }}
            </option>
            <option value="1" {{ isset($branch) && $branch->is_active == 1 ? 'selected' : old('is_active') }}>
                {{ __('index.active') }}</option>
            <option value="0" {{ isset($branch) && $branch->is_active == 0 ? 'selected' : old('is_active') }}>
