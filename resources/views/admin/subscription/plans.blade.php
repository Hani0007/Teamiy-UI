...
.plan-footer {
    ...
    gap: 10px;
    align-items: center;
    margin-top: auto; /* ensure same height cards */
}
...
<section class="content">
    @if (auth()->user()->trial_expiry && \Carbon\Carbon::parse(auth()->user()->trial_expiry)->lt(\Carbon\Carbon::today()))
        <div class="alert alert-danger text-center">
            Your {{ $planName }} plan has expired. To continue using the dashboard and access all features smoothly, please upgrade your plan.
        </div>
    @endif
            ...
            <div class="plans-container">
                ...
                <div class="plan-card">
                    ...
                    <div class="plan-price-container">
                        <span class="plan-price" data-month="{{ $plan->price_per_month }}" data-year="{{ $plan->price_per_year }}">
    ...
    @php
        $basicFeatures = $plans->firstWhere('name', 'Basic')->packageModules->pluck('module.name')->toArray();
        $standardFeatures = $plans->firstWhere('name', 'Standard')->packageModules->pluck('module.name')->toArray();
...
$(document).ready(function () {
    ...
    function updatePrice(card) {
        ...
        const basePrice = cycle === 'monthly' ? parseFloat(priceEl.data('month')) : parseFloat(priceEl.data('year'));
        const total = basePrice * employees;
        priceEl.text('€' + total.toFixed(2) + '/employee');
        card.find('.plan-cycle').text(cycle === 'monthly' ? '/ Month' : '/ Year');
    }

    $('.employee-count').on('keyup change', function () { updatePrice($(this).closest('.plan-card')); });
    ...
    $('.subscribe-btn').on('click', async function () {
        ...
        try {
            const res = await fetch("{{ route('admin.stripe.create.subscription') }}", {
                ...
                body: JSON.stringify({ plan_id: planId, cycle, total_employees: employees })
            });
            const result = await res.json();
            if (!result.client_secret || !result.subscription_id) throw new Error("Payment initialization failed");
    ...
    $('#payNowBtn').on('click', async function () {
        if (!clientSecret || !subscriptionId) { $('#card-errors').text('Payment not initialized.'); return; }
        $('#card-errors').text('');
        $(this).prop('disabled', true).text('Processing...');

        const payment = await stripe.confirmCardPayment(clientSecret, { payment_method: { card: cardNumber } });
